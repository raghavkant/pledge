...
| 2026-09-26 | (Owner's instruction) Both projects moved off the iCloud-synced Desktop for good: this repo now lives at `~/Developer/pledge-website`, the app project at `~/Developer/citizenship-test`. Updated the app-project path in `CLAUDE.md`, `docs/rules.md`, `docs/app-project-changes.md`, and the default in `scripts/export-questions.mjs`, `scripts/build-icons.mjs`, `scripts/check-paid-leak.mjs`. Older dated entries above still say `~/pledge-work` / `~/Desktop/pledge-website` because that was true when they were written; `docs/overnight-report.md` and `docs/run-2-summary.md` got a "superseded" note pointing here. |
